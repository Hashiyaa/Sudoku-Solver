...
class Game extends React.Component {
    ...
    handleSolve() {
        let sol = calculate(this.state.config);
        this.setState({
            config: sol,
        });
        sol.forEach((row, i) => {
            row.forEach((square, j) => {
                if (square.length == 1 && this.state.typeConfig[i][j] == "unplaced") {
                    let typeConfig = this.state.typeConfig.slice();
...
class Square extends React.Component {
        ...
        return numbers.map((row) => {
            return (
                <tr key={row} >
                {
                    numbers.map((col) => {
                        let numberStr = "";
                        let number = 3 * row + col + 1;
                        if (this.props.value.includes(number)) {
    ...
    render() {
        if (this.props.value.length == 1) {
            ...
            return (
                <input className="square" type="text" min="1" max="9" value=""
                    onChange={event => this.props.onChange(this.props.row, this.props.col, Number(event.target.value))}
                    onFocus={() => event.target.type = "number"}
                    onBlur={() => event.target.type = "text"}
                /> 
